...
Write-Host "`nYou selected: $($target.Name)"
Write-Host "Current Status: $(if ($target.Enabled) { 'Enabled' } else { 'Disabled' })"
...
try {
    ...
    Write-Host "`n✔ Local user '$($target.Name)' has been DISABLED." -ForegroundColor Green
}
catch {
